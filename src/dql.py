...
class DQLTrainer:
    ...
    def __init__(self, env, model_path=None, buffer_capacity=50000):
        ...
        self.q1 = self.QNetwork(input_dim, output_dim).to(self.device)
        self.q2 = self.QNetwork(input_dim, output_dim).to(self.device)
        self.q2.load_state_dict(self.q1.state_dict())
        self.q2.eval()

        self.optimizer = optim.Adam(self.q1.parameters(), lr=1e-4)
    ...
    def _load_model(self):
        self.q1.load_state_dict(torch.load(self.model_path, map_location=self.device))
        self.q1.eval()
    ...
    def train(self, episodes=300, gamma=0.99, epsilon_start=1.0, epsilon_end=0.05,
        ...
        for episode in trange(episodes, desc="Training Episodes"):
            ...
            if total_reward > max_reward:
                ...
                if self.model_path:
                    torch.save(self.q1.state_dict(), self.model_path)
        ...
        return self.q1, best_state_action_pairs, reward_history, action_counter
    ...
    def _optimize_model(self, batch_size, gamma):
        ...
        with torch.no_grad():
            next_q = self.q2(next_states).gather(1, self.q1(next_states).argmax(1, keepdim=True))
            target_q = rewards + gamma * next_q * (1 - dones)

        current_q = self.q1(states).gather(1, actions)
